...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    auto w1 = dict.find(word1_);
    auto w2 = dict.find(word2_);
    if (w1 == dict.end() || w2 == dict.end()) {
        return false;
    }

    //pronunciations
    const vector<string>& pro1 = w1->second;
    const vector<string>& pro2 = w2->second;

    if (pro1.size() != pro2.size()) {
        return false;
    }

    for (size_t i = 0; i < pro1.size(); i++) {
        if (pro1[i] != pro2[i]) {
            return false;
        }
    }
