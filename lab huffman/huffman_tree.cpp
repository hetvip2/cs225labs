                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    TreeNode* single1 = singleQueue.front();
    TreeNode* merge1 = mergeQueue.front();

    // remove ties
    if (single1->freq.getFrequency() <= merge1->freq.getFrequency()) {
        singleQueue.pop();
        return single1;
    } else {
        mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
        auto left = removeSmallest(singleQueue, mergeQueue);
        auto right = removeSmallest(singleQueue, mergeQueue);

        // both nodes to mergequeue
        TreeNode* parent = new TreeNode(Frequency('\0', left->freq.getFrequency() + right->freq.getFrequency()));
        parent->left = left;
        parent->right = right;
    ...
    root_ = std::move(mergeQueue.front());
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        if (!current->left && !current->right) {
            ...
            current = root_; //go back to root for next char
        }
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.getNextBit()) {
        ...
        return new TreeNode(Frequency(c, 0));
    } else {
        // node
        TreeNode* node = new TreeNode(Frequency('\0', 0));
